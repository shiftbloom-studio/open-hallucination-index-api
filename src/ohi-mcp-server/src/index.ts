...
const tools: Tool[] = [
  ...
  {
    name: "get_summary",
    description: "Get Wikipedia article summary (plain text)",
    inputSchema: {
      type: "object",
      properties: {
        title: { type: "string", description: "Article title" },
  ...
  },
  {
    ...
    inputSchema: {
      type: "object",
      properties: {
        query: { type: "string", description: "User question or task for ranking" },
  ...
  {
    name: "query-docs",
    description: "Retrieve Context7 documentation context for a library",
    inputSchema: {
      type: "object",
      properties: {
...
async function main() {
  ...
  if (transportMode === "stdio") {
    ...
    // =========================================================================
    // Direct HTTP API (simpler than SSE for request-response patterns)
    // =========================================================================
    ...
    // Call a tool directly via HTTP POST
    app.post("/api/call", async (req: Request, res: Response) => {
      ...
      try {
        // Execute the tool using the same handler as MCP
    ...
    // Convenience endpoint for search_all
    app.post("/api/search", async (req: Request, res: Response) => {
      const { query, limit = 5, sources } = req.body;
      ...
      try {
        const result = await executeToolCall("search_all", { query, limit, sources });
        res.json(result);
      } catch (error) {
        const message = error instanceof Error ? error.message : "Unknown error";
        res.status(500).json({ error: message });
