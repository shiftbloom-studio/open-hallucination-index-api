...
export class HttpClient {
  ...
  async request<T = unknown>(url: string, options: RequestOptions = {}): Promise<HttpResponse<T>> {
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      try {
        console.log(`[HttpClient] Requesting ${method} ${fullUrl}`);
        ...
        // Don't retry on client errors, but DO retry on 429 (Too Many Requests)
        if (lastError.message.includes("HTTP 4") && !lastError.message.includes("HTTP 429")) {
          throw lastError;
        }

        if (attempt < MAX_RETRIES) {
          // If 429, wait a bit longer
          const delay = lastError.message.includes("HTTP 429") 
