...
class EvidenceSource(StrEnum):
    ...
    WIKIMEDIA_REST = auto()  # Wikimedia REST API
    WIKIDATA = auto()  # Wikidata SPARQL
    OPENCITATIONS = auto()  # OpenCitations API
    OPENALEX = auto()  # OpenAlex API
    NCBI = auto()  # NCBI/PubMed API
    MEDIAWIKI = auto()  # MediaWiki API
    EUROPE_PMC = auto()  # Europe PMC API
    DBPEDIA = auto()  # DBpedia SPARQL
    CROSSREF = auto()  # Crossref API
    CLINICALTRIALS = auto()  # ClinicalTrials.gov API
