        ...
        try:
            ...
            if "processed_files" in data:
                data["processed_filenames"] = [
                    os.path.basename(p) for p in data.pop("processed_files")
                ]
            if "current_file" in data:
                data["current_filename"] = (
                    os.path.basename(data.pop("current_file")) if data["current_file"] else None
                )
        ...
        for event, elem in context:
            ...
            if event == "start" and tag == "page":
                ...
                if tag == "page":
                    ...
                    elif tag == "id" and "id" not in current:
                        current["id"] = elem.text  # Page ID
                    elif tag == "id":
                        current["revision"] = elem.text  # Rev ID
                    elif tag == "ns":
        ...
        for dump_file in files_to_process:
            ...
            with ThreadPoolExecutor(max_workers=args.workers) as executor:
                ...
                while not shutdown_event.is_set():
                    ...
                    if not chunk:
                        break  # Sollte oben gefangen werden, aber sicher ist sicher
                    ...
                    if processed_articles:
                        ...
                        logger.info(
                            f"Importiert: {len(processed_articles)} Artikel (Letzte ID: {current_max_id})"
                        )
