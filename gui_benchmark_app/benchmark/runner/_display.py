...
def _detect_terminal_capabilities() -> dict[str, bool]:
    ...
    # Detect capabilities
    has_unicode = sys.stdout.encoding and sys.stdout.encoding.lower() in ("utf-8", "utf8")
...
class LiveBenchmarkDisplay:
    ...
    def _render_kpis(self) -> Columns:
        ...
        cards = [
            self._kpi_card(
                f"{icons['speed']} Throughput", f"{throughput:.2f}/s", style="bold cyan"
            ),
            ...
            self._kpi_card(
                f"{icons['chart']} Done", f"{self.stats.current_completed}", style="bold"
            ),
            self._kpi_card(
                f"{icons['warn']} Exceptions", str(self.stats.current_errors), style=error_style
            ),
        ]
