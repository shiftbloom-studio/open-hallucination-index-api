...
class ComparisonBenchmarkConfig:
    ...
    complete_samples_per_dataset: int = (
        250  # Increased to ensure hallucinations are included (shuffled datasets)
    )
    ...
    output_dir: Path = field(default_factory=lambda: Path("benchmark_results/comparison"))
    chart_dpi: int = 200
    ...
    def from_env(cls) -> "ComparisonBenchmarkConfig":
        ...
        return cls(
            ...
            complete_mode=os.getenv("BENCHMARK_COMPLETE_MODE", "false").lower() == "true",
            complete_samples_per_dataset=int(os.getenv("COMPLETE_SAMPLES_PER_DATASET", "200")),
            complete_min_verifications=int(os.getenv("COMPLETE_MIN_VERIFICATIONS", "800")),
            complete_statistical_significance=os.getenv(
            ...
            output_dir=Path(os.getenv("BENCHMARK_OUTPUT_DIR", "benchmark_results/comparison")),
    ...
    def validate(self) -> list[str]:
        ...
        if not self.hallucination_dataset.exists():
            warnings.append(f"Hallucination dataset not found: {self.hallucination_dataset}")

        if self.extended_dataset and not self.extended_dataset.exists():
            warnings.append(f"Extended dataset not found: {self.extended_dataset}")
